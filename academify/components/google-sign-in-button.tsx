"use client";

import { useEffect, useState } from "react";
import { authClient } from "@/lib/auth-client";

type AuthConfig = {
  googleEnabled: boolean;
  googleCallbackUrl: string;
};

export default function GoogleSignInButton({
  label = "Continue with Google",
  callbackURL = "/dashboard",
}: {
  label?: string;
  callbackURL?: string;
}) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [config, setConfig] = useState<AuthConfig | null>(null);

  useEffect(() => {
    fetch("/api/auth/config")
      .then((res) => res.json())
      .then((data: AuthConfig) => setConfig(data))
      .catch(() => setConfig({ googleEnabled: false, googleCallbackUrl: "" }));
  }, []);

  const handleGoogleSignIn = async () => {
    if (!config?.googleEnabled) {
      setError(
        "Google sign-in is not configured. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env.local, then restart npm run dev."
      );
      return;
    }

    setLoading(true);
    setError(null);
    try {
      const result = await authClient.signIn.social({
        provider: "google",
        callbackURL,
      });

      const url = result.data?.url;
      if (url) {
        window.location.assign(url);
        return;
      }

      if (result.error) {
        throw new Error(result.error.message || "Google sign-in failed");
      }

      setError("Google did not return an authorization URL. Check the server console and /api/auth/config.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Google sign-in failed");
    } finally {
      setLoading(false);
    }
  };

  if (config && !config.googleEnabled) {
    return (
      <p className="text-center text-xs text-gray-400">
        Google sign-in is off — set <code className="text-gray-500">GOOGLE_CLIENT_*</code> in{" "}
        <code className="text-gray-500">.env.local</code> and restart the server.
      </p>
    );
  }

  return (
    <div className="space-y-2">
      <button
        type="button"
        onClick={handleGoogleSignIn}
        disabled={loading || !config}
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-60"
      >
        <svg className="h-4 w-4" viewBox="0 0 24 24" aria-hidden>
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        {loading ? "Redirecting..." : label}
      </button>
      {error && <p className="text-center text-xs text-red-500">{error}</p>}
      {process.env.NODE_ENV === "development" && config?.googleCallbackUrl && (
        <p className="text-center text-[10px] leading-relaxed text-gray-400">
          Redirect URI: <span className="break-all">{config.googleCallbackUrl}</span>
        </p>
      )}
    </div>
  );
}
